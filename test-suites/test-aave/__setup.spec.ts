...
import { Dexs } from '../../markets/aave/commons';
import { pack, keccak256 } from '@ethersproject/solidity';
import _ from 'lodash';
import { VeFactory } from '../../types';
...
const buildTestEnv = async (deployer: Signer, secondaryWallet: Signer) => {
  ...
  await setInitialAssetPricesInOracle(
    ...
    {
      KLAPKLAY: mockTokens.KLAPKLAY.address,
  ...
  const lendingPoolConfigurator = await addressesProvider.getLendingPoolConfigurator();
  const token = await deployGeistToken(false);
  //const tokenLocker = await deokit

  // GEIST STAKING SECTION STARTS HERE

  const mockUniswapRouter = await deployMockUniswapRouter(mockTokens.KLAPKLAY.address);

  // MasterChef = LP Staking Rewards
  // Chef Incentives = Pool Rewards
  const ve = await deployVe(token.address, mockTokens.KLAPKLAY.address);
  const multiFeeDistribution = await deployMultiFeeDistribution(token.address, ve.address, false);

  //console.log("from, ", await getFirstSigner())
  // VE DEPLOYMENT SECTION HERE
  ...
  const masterchefController = await deployMasterChefController(
    lendingPoolConfigurator,
    multiFeeDistribution.address,
    false
  );
  //const podl = await deployPODL(incentivesController.address, multiFeeDistribution.address, true)
  //await waitForTx(await multiFeeDistribution.setIncentivesController(incentivesController.address))
  await waitForTx(
    await multiFeeDistribution.setMinters([
      masterchefController.address,
      incentivesController.address,
    ])
  );
  await waitForTx(await multiFeeDistribution.setIncentivesController(incentivesController.address));
  await waitForTx(await multiFeeDistribution.transferOwnership(lendingPoolConfigurator));
  await waitForTx(await ve.setVoter(incentivesController.address));
  await waitForTx(await ve.setMultiFeeContract(multiFeeDistribution.address));
  await waitForTx(await incentivesController.start());
  await waitForTx(await masterchefController.start());
  await waitForTx(await token.setMinter(multiFeeDistribution.address));
  await waitForTx(await masterchefController.addPool(mockTokens.KLAPKLAY.address, 10));

  // GEIST STAKING SECTION ENDS HERE
