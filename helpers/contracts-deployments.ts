...
import {
  ...
  MasterChefFactory,
  TreasuryFactory,
  FairAssetReservesFactory,
  Lperc20,
  Lperc20Factory,
  MockFairAssetReservesFactory,
  VeFactory,
  MultiRewarderFactory,
} from '../types';
import {
  withSaveAndVerify,
...
export const deployTreasury = async (
  ...
  const treasuryProxy = await deployProxy(new TreasuryFactory(await getFirstSigner()), [
    stakingToken,
    lendingPoolConfigurator,
    devPercent,
    dev
  ])
  await registerContractInJsonDb(eContractid.Treasury, treasuryProxy);
  return treasuryProxy;
};

export const deployMultiRewarder = async (
  deployProxy: any,
  verify?: boolean
) => {
  const multirewarderProxy = await deployProxy(new MultiRewarderFactory(await getFirstSigner()), [
    
  ])
  await registerContractInJsonDb(eContractid.MultiRewarder, multirewarderProxy);

  return multirewarderProxy;
};
...
export const deployIncentivesController = async (
  ...
  veContract: string,
  _dialutingRepartition: number,
  rewarder: string,
  deployProxy: any,
  verify?: boolean
) => {
  const chefProxy = await deployProxy(new ChefIncentivesControllerFactory(await getFirstSigner()), [
    [0],
    ['10000000000000'],
    poolConfigurator,
  ...
  await registerContractInJsonDb(eContractid.ChefIncentivesController, chefProxy);

  return chefProxy;
};

export const deployMasterChefController = async (
  poolConfigurator: string,
  multiFeeDistribution: string,
  veAddress: string,
  _dialutingRepartition: number,
  ...
  const masterchefProxy = await deployProxy(new MasterChefFactory(await getFirstSigner()), [
    [0],
    ['10000000000000'],
    poolConfigurator,
    multiFeeDistribution,
  ...
  await registerContractInJsonDb(eContractid.MasterChef, masterchefProxy);

  return masterchefProxy;
};

export const deployVe = async (tokenAddress: string, lpToken: string, fairAssetContract: string, deployProxy: any, verify?: boolean) => {
  const veProxy = await deployProxy(new VeFactory(await getFirstSigner()), [
    tokenAddress,
    lpToken,
    fairAssetContract,
    fairAssetContract
  ])
  await registerContractInJsonDb(eContractid.VeNFT, veProxy);
...
export const deployMultiFeeDistribution = async (
  ...
  const multifeeProxy = await deployProxy(new MultiFeeDistributionFactory(await getFirstSigner()), [
    address, 
    ve, 
    wrappedNativeToken, 
    rewardPercentage, 
  ...
  await registerContractInJsonDb(eContractid.MultiFeeDistribution, multifeeProxy);

  return multifeeProxy;
};

export const deployFairAssetContract = async (
  priceOracle: string,
  lpToken: string,
  verify?: boolean
) =>
  withSaveAndVerify(
    await new FairAssetReservesFactory(await getFirstSigner()).deploy(priceOracle, lpToken),
    eContractid.FairAssetContract,
    [priceOracle, lpToken],
    verify
  );

export const deployMockFairAssetContract = async (
  priceOracle: string,
  lpToken: string,
  verify?: boolean
) =>
  withSaveAndVerify(
    await new MockFairAssetReservesFactory(await getFirstSigner()).deploy(priceOracle, lpToken),
    eContractid.FairAssetContract,
    [priceOracle, lpToken],
    verify
  );

export const deployGeistToken = async (verify?: boolean) =>
  withSaveAndVerify(
    await new KlapTokenFactory(await getFirstSigner()).deploy('10000000000000000000000000000'),
    eContractid.GeistToken,
...
export const deployLPToken = async (
  args: [string, string, string],
  token0: string,
  token1: string,
  verify?: boolean
  ...
  withSaveAndVerify(
    await new Lperc20Factory(await getFirstSigner()).deploy(...args, token0, token1),
    eContractid.LPToken,
    [...args, token0, token1],
    verify
  );
...
export const deployMockUniswapRouter = async (address: string, verify?: boolean) =>
  withSaveAndVerify(
    await new MockUniswapV2Router02Factory(await getFirstSigner()).deploy(address),
