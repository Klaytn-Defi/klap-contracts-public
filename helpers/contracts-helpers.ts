...
export const getParamPerNetwork = <T>(param: iParamsPerNetwork<T>, network: eNetwork) => {
  const { main, ropsten, kovan, coverage, buidlerevm, tenderly, rinkeby } =
    param as iEthereumParamsPerNetwork<T>;
  const { matic, mumbai } = param as iPolygonParamsPerNetwork<T>;
  const { xdai } = param as iXDaiParamsPerNetwork<T>;
  const { avalanche, fuji } = param as iAvalancheParamsPerNetwork<T>;
  const { baobab, cypress } = param as iKlaytnParamsPerNetwork<T>;
  if (process.env.FORK) {
  ...
  switch (network) {
    ...
    case eAvalancheNetwork.fuji:
      return fuji;
    case eKlaytnNetwork.baobab:
      return baobab;
    case eKlaytnNetwork.cypress:
      return cypress;
  }
};
