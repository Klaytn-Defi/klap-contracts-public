...
export const getMultiFeeDistribution = async (address?: tEthereumAddress) =>
  await MultiFeeDistributionFactory.connect(
    address ||
      (
...
export const getMasterChef = async (address?: tEthereumAddress) =>
  await MasterChefFactory.connect(
    address || (await getDb().get(`${eContractid.MasterChef}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );

export const getGeistToken = async (address?: tEthereumAddress) =>
  await KlapTokenFactory.connect(
    address || (await getDb().get(`${eContractid.GeistToken}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );

export const getLPToken = async (address?: tEthereumAddress) =>
  await Lperc20Factory.connect(
    address || (await getDb().get(`${eContractid.LPToken}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );
...
export const getPairsTokenAggregator = (
  ...
  console.log('pairs token aggregator');
...
export const getVe = async (address?: tEthereumAddress) =>
  await VeFactory.connect(
    address || (await getDb().get(`${eContractid.VeNFT}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );

export const getChefIncentivesController = async (address?: tEthereumAddress) =>
  await ChefIncentivesControllerFactory.connect(
    address ||
      (
...
export const getMultiRewarder = async (address?: tEthereumAddress) =>
  await MultiRewarderFactory.connect(
    address ||
      (
        await getDb().get(`${eContractid.MultiRewarder}.${DRE.network.name}`).value()
      ).address,
    await getFirstSigner()
  );
