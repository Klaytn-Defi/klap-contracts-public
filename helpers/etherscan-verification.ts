...
export const SUPPORTED_ETHERSCAN_NETWORKS = ['main', 'ropsten', 'kovan', 'matic', 'mumbai', 'rinkeby'];
...
export const runTaskWithRetry = async (
  ...
  try {
    ...
    if (okErrors.some((okReason) => (error as any).message.includes(okReason))) {
      console.info('[ETHERSCAN][INFO] Skipping due OK response: ', (error as any).message);
      return;
    }

    if (fatalErrors.some((fatalError) => (error as any).message.includes(fatalError))) {
      console.error(
    ...
    console.error('[ETHERSCAN][ERROR]', (error as any).message);
    console.log();
    console.info(`[ETHERSCAN][[INFO] Retrying attemps: ${counter}.`);
    if ((error as any).message.includes(unableVerifyError)) {
      console.log('[ETHERSCAN][WARNING] Trying to verify via uploading all sources.');
      delete params.relatedSources;
    }
