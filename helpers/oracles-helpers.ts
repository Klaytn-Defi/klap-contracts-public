...
import { PriceFeed, PriceFeedFactory } from '../types';
...
export const deployPriceFeed = async (chainlinkOracle: string) => {
  return withSaveAndVerify(
    await new PriceFeedFactory(await getFirstSigner()).deploy(
      chainlinkOracle,
      1000,
      "un"
    ),
...
export const deployAllPriceFeedors = async (aggregators: string[]) => {
  const priceFeedors: any[] = [];
  for (let aggregator of aggregators) {
    const priceFeed = await deployPriceFeed(aggregator)
    priceFeedors.push(priceFeed.address)
  }
  return priceFeedors
}
